...
# ==============================================================================
# 🕵️‍♂️ LÓGICA DEL BUSCADOR ESPECÍFICO (SIN TRY/EXCEPT)
# ==============================================================================
if boton_buscar:
    if not texto_input:
    ...
    # Mensaje temporal para confirmar que empieza el proceso
    st.caption(f"⚙️ Procesando datos para: {ticker_encontrado}...")
    ...
    with st.spinner(f"Redactando informe para {nombre_bonito}..."):
        ...
        if df_hist.empty:
            st.error(f"❌ Error: No se han podido descargar datos para '{ticker_encontrado}'. Puede que el mercado esté cerrado o el ticker sea incorrecto.")
            st.stop()
        ...
        with gc1:
            ...
            # Si falla el gráfico, lo capturamos aquí localmente
            try:
                fig = graficos.crear_grafico_lineas(df_hist)
                st.pyplot(fig)
            except Exception as e:
                st.error(f"Error pintando el gráfico: {e}")
...
# ==============================================================================
    ...
    # Aquí sí dejamos el try/except global porque es un proceso masivo y no queremos que pare por una sola empresa
    try:
        df_todos = datos.descargar_datos(datos.EMPRESAS_SELECCIONADAS)
        factor_eur = datos.obtener_precio_dolar()
    except Exception as e:
        st.error(f"Error grave descargando datos masivos: {e}"); st.stop()
